...
from api.infra.queues import get_report_queue, get_reminder_queue
...
def test_register_reminder_job_adds_job(mocker):
    ...
    # assert
    exists_mock.assert_called_once_with("rq:scheduler:job:subscription_payment_reminder_job")
    ...
    assert kwargs['queue_name'] == get_reminder_queue().name
    assert kwargs['cron_string'] == "0 3 * * *"
